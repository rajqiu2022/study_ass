---
name: study-assistant
description: 学习助手 - 通过 AI 助手进行知识库管理、智能记账、联网搜索、笔记管理等操作
version: 1.0.0
author: rajqiu
---

# 学习助手 (Study Assistant)

你是一个连接到远程学习助手系统的 AI 技能。通过 HTTP API 与部署在腾讯云服务器上的学习助手后端交互，实现以下功能：

- 💬 **AI 对话**：与 AI 助手进行多轮对话，支持上下文记忆
- 📝 **知识库管理**：保存笔记、查看笔记列表
- 💰 **智能记账**：自然语言记账（如"花了30块吃午饭"），自动解析并保存
- 🌐 **联网搜索**：搜索互联网获取最新信息
- 🔗 **链接分析**：发送链接自动抓取分析网页内容
- 📊 **对话管理**：创建新对话、查看对话列表、查看历史记录

## 配置信息

- **API 地址**: `https://testcase.work:8088`
- **认证方式**: Bearer Token（从学习助手管理后台生成）
- **Token 配置**: 写在同目录下的 `config.json` 文件中

### config.json 格式
```json
{
  "api_url": "https://testcase.work:8088",
  "token": "你的 Bot API Token",
  "user_id": "你在网站注册的用户名",
  "user_name": "OpenClaw"
}
```
> ⚠️ **`user_id` 必须设置为你在学习助手网站上注册的真实用户名**（不是默认的 `openclaw_user`）。否则记账、笔记数据会记到错误的账号下，且无法修改/删除 Web 端创建的记录。

## API 端点

> 所有需要认证的接口都必须带上：
> - `Authorization: Bearer <TOKEN>` — 认证 Token
> - `X-Bot-User: <user_id>` — 用户标识（或在请求体中传 `user_id`）
>
> **数据隔离**：每个 user_id 只能查看和操作自己的数据。

---

### 1. 健康检查
```
GET /bot-api/ping
```
无需认证，用于检查服务是否在线。

---

### 2. 用户查询（⚠️ 必须优先调用）
```
GET /bot-api/user/check?username=张三
```
需要 `Authorization: Bearer <TOKEN>`，**不需要** `X-Bot-User` header。

**响应 — 用户存在：**
```json
{
    "exists": true,
    "username": "张三",
    "bot_user_id": "张三",
    "role": "user",
    "scene": "general",
    "created_at": "2026-03-10 08:00",
    "note_count": 15,
    "conversation_count": 5
}
```
> ⚠️ `bot_user_id` 是你后续请求中应使用的 `X-Bot-User` header 值，确保数据写入正确的用户账号。

**响应 — 用户不存在：**
```json
{
    "exists": false,
    "username": "张三",
    "register_url": "https://testcase.work:8088",
    "message": "用户 \"张三\" 不存在，请先注册账号"
}
```

> ⚠️ **重要**：在使用知识库、记账等任何需要用户身份的功能之前，**必须先调用此接口确认用户账号存在**。如果不存在，告知对方先去 https://testcase.work:8088 注册。

---

### 2.5 获取当前登录用户信息（Web 前端用）
```
GET /bot-api/user/me
```
基于 Flask-Login 的 session 认证（无需 Bearer Token），返回当前登录用户信息。

**响应 — 已登录：**
```json
{
    "logged_in": true,
    "user_id": 1,
    "username": "张三",
    "role": "user",
    "scene": "general",
    "bio": "",
    "interests": "编程,AI",
    "current_learning": "Python",
    "created_at": "2026-03-10 08:00",
    "note_count": 15,
    "conversation_count": 5
}
```

**响应 — 未登录：**
```json
{
    "logged_in": false,
    "message": "未登录"
}
```

> 此接口主要给 Web 前端使用，用于在 AI 助手页面显示当前用户信息和确认记账/笔记数据归属。

---

### 3. AI 对话（核心）
```
POST /bot-api/chat
```
> ⚠️ **重要**：chat 接口会**自动检测记账意图**（如"花了30吃饭"、"把那笔改成50"等），在服务端自动执行记账的新增/修改/删除操作。
> 因此，**即使是普通对话，只要消息可能涉及记账，`X-Bot-User` 也必须设置为用户在网站注册的真实用户名**！
> 否则记账数据会归属到错误的账号，导致修改/删除操作失败（因为查不到对应记录）。

**请求体：**
```json
{
    "message": "用户的消息",
    "user_id": "openclaw_user",
    "conversation_id": 123,
    "enable_search": false,
    "user_name": "OpenClaw用户"
}
```
- `conversation_id` 可选，带上则续接已有对话
- `enable_search` 可选，`true` 表示强制联网搜索
- **`user_id`** 和 `X-Bot-User` header 应设为用户在网站注册的真实用户名（通过 user-check 获取的 `bot_user_id`）

**响应：**
```json
{
    "response": "AI 回复内容",
    "conversation_id": 123,
    "conversation_title": "对话标题",
    "actions": ["已搜索: xxx"],
    "finance_record": {
        "type": "支出",
        "amount": 30.0,
        "category": "餐饮",
        "description": "午饭",
        "action": "add",
        "date": "2026-03-17"
    }
}
```

---

### 4. 对话管理

| 接口 | 方法 | 说明 |
|------|------|------|
| `/bot-api/conversations` | GET | 获取对话列表（最近20条） |
| `/bot-api/conversations/new` | POST | 新建对话，body: `{"user_id":"...", "title":"新对话"}` |
| `/bot-api/conversations/<id>/history` | GET | 获取指定对话的聊天记录，可选 `?limit=20` |

---

### 5. 知识库（笔记）管理 — 完整 CRUD

#### 5.1 查询笔记列表
```
GET /bot-api/notes?category=study&tag=python&q=装饰器&page=1&per_page=20
```
**查询参数（均可选）：**
- `category` — 按分类筛选：`work` / `study` / `life` / `general`
- `tag` — 按标签模糊搜索
- `q` — 按标题和内容关键词搜索
- `page` / `per_page` — 分页（默认 page=1, per_page=20, 最大50）

**响应：**
```json
{
    "notes": [
        {
            "id": 1,
            "title": "Python装饰器笔记",
            "category": "study",
            "tags": "python,编程",
            "folder": "/",
            "source_type": "bot",
            "created_at": "2026-03-17 10:00",
            "updated_at": "2026-03-17 10:30",
            "content_preview": "装饰器的核心是闭包..."
        }
    ],
    "total": 15,
    "page": 1,
    "pages": 1
}
```

#### 5.2 查看笔记详情
```
GET /bot-api/notes/<id>
```
返回完整的笔记内容。

#### 5.3 创建笔记
```
POST /bot-api/notes
```
```json
{
    "title": "笔记标题",
    "content": "笔记内容（支持 Markdown）",
    "category": "study",
    "tags": "tag1,tag2",
    "folder": "/"
}
```
- `title` 可选，不传会自动从内容第一行提取
- `category` 可选，默认 `general`

#### 5.4 修改笔记
```
PUT /bot-api/notes/<id>
```
```json
{
    "title": "新标题",
    "content": "新内容",
    "category": "work",
    "tags": "new_tag"
}
```
只传需要修改的字段即可，不传的字段保持不变。

#### 5.5 删除笔记
```
DELETE /bot-api/notes/<id>
```

> 兼容旧接口：`POST /bot-api/save-note` 仍可用，等价于 `POST /bot-api/notes`。

---

### 6. 记账管理 — 完整 CRUD

#### 6.1 查询记账列表
```
GET /bot-api/finance?type=expense&category=餐饮&start_date=2026-03-01&end_date=2026-03-31&q=午饭&page=1
```
**查询参数（均可选）：**
- `type` — 按类型筛选：`expense`（支出）/ `income`（收入）
- `category` — 按分类筛选
- `start_date` / `end_date` — 日期范围，格式 `YYYY-MM-DD`
- `q` — 按描述关键词搜索
- `page` / `per_page` — 分页

**响应：**
```json
{
    "records": [
        {
            "id": 1,
            "type": "支出",
            "record_type": "expense",
            "amount": 88.0,
            "category": "购物",
            "description": "买了一本Python书",
            "date": "2026-03-17",
            "source": "ai",
            "created_at": "2026-03-17 14:00"
        }
    ],
    "total": 30,
    "page": 1,
    "pages": 2,
    "summary": {
        "total_expense": 1500.0,
        "total_income": 5000.0,
        "balance": 3500.0
    }
}
```
> `summary` 是当前筛选条件下的汇总统计。

#### 6.2 查看单条记账
```
GET /bot-api/finance/<id>
```

#### 6.3 新增记账
```
POST /bot-api/finance
```
```json
{
    "record_type": "expense",
    "amount": 88.0,
    "category": "购物",
    "description": "买了一本书",
    "date": "2026-03-17"
}
```
- `record_type` 必填：`expense` 或 `income`
- `amount` 必填：金额，大于 0
- `category` 必填：分类名称
- `description` 可选
- `date` 可选，默认当天

#### 6.4 修改记账
```
PUT /bot-api/finance/<id>
```
```json
{
    "amount": 99.0,
    "category": "教育",
    "description": "买了两本书"
}
```
只传需要修改的字段。

#### 6.5 删除记账
```
DELETE /bot-api/finance/<id>
```

#### 6.6 获取分类列表
```
GET /bot-api/finance/categories
```
返回系统预设的支出和收入分类：
```json
{
    "expense_categories": ["餐饮","交通","购物","娱乐",...],
    "income_categories": ["工资","奖金","兼职","理财",...]
}
```

---

## 使用规则

当用户发送消息时，请按以下规则处理：

### ⚠️ 用户身份确认（最高优先级）

**数据归属原则**：记账和知识库的数据属于具体的用户账号。OpenClaw 代替用户操作时，必须使用该用户在网站上注册的真实账号，否则数据会记到错误的账号下。

在执行任何**知识库操作**（笔记增删改查）或**记账操作**（记账增删改查）之前，**必须先确认用户账号**：

1. **询问用户名**：如果还不知道对方的账号名，先问："请问你在学习助手网站上注册的用户名是什么？"
2. **调用查询接口**：`GET /bot-api/user/check?username=用户名`
3. **判断结果**：
   - 如果 `exists: true` → **取响应中的 `bot_user_id` 字段**，用它作为后续所有请求的 `X-Bot-User` header 值或 `user_id` 字段值。这样数据会正确写入该用户的账号下。
   - 如果 `exists: false` → 告知对方："你的账号还未注册，请先访问 https://testcase.work:8088 注册一个账号，然后再来找我。"**不要继续执行操作**
4. **记住身份**：同一次会话中，确认过一次就够了，后续操作不需要重复询问

> ⚠️ **关键**：一定要用 `bot_user_id`（即用户在网站注册的真实用户名）作为 `X-Bot-User`，而不是随便取一个标识。这决定了记账和笔记数据归属到哪个账号。
>
> ⚠️ **特别注意 chat 接口**：`/bot-api/chat` 会在服务端**自动检测记账意图**（如"花了30吃饭"、"把那笔改成50"），并自动执行记账的新增/修改/删除。因此，**所有对话请求都建议使用正确的用户身份**，以免记账数据归属错误。如果 X-Bot-User 不是用户在网站上的真实用户名，修改/删除记账操作**会静默失败**（因为按 user_id 查不到记录，会 fallthrough 变成新增）。
>
> 简单来说：**先确认身份，再进行所有操作（包括普通聊天）。**

### 指令识别

| 用户说的话 | 动作 |
|-----------|------|
| `/新对话` 或 `/new` | 调用新建对话接口，清除上下文 |
| `/搜索 xxx` 或 `/search xxx` | 调用对话接口，`enable_search: true` |
| `/记笔记 内容` 或 `/save 内容` | 调用 `POST /bot-api/notes` 创建笔记 |
| `/笔记列表` 或 `/notes` | 调用 `GET /bot-api/notes` 查看笔记列表 |
| `/搜笔记 关键词` | 调用 `GET /bot-api/notes?q=关键词` |
| `/账单` 或 `/finance` | 调用 `GET /bot-api/finance` 查看本月账单 |
| `/历史` 或 `/history` | 调用对话列表接口 |
| `/帮助` 或 `/help` | 显示帮助信息（见下方） |
| 其他所有消息 | 调用对话接口进行 AI 对话 |

### ⚠️ 链接保存 — 必须两步操作（最重要的规则）

**问题背景**：`/bot-api/chat` 接口在收到链接时，会自动抓取并分析内容、返回 AI 整理的摘要，但 **不会** 自动保存为笔记。AI 只是"说"保存了，实际上数据库里没有任何记录。

**正确的保存流程**：当用户发送链接 + 保存意图（如"帮我保存"、"整理保存到知识库"、"收藏一下"等），必须分两步完成：

#### 第一步：调用 `/bot-api/chat` 获取 AI 分析内容
```
POST /bot-api/chat
{
    "message": "https://example.com/article 帮我整理保存",
    "user_id": "用户名"
}
```
响应中会包含 AI 整理的内容（`response` 字段），以及 `actions` 中的抓取记录。

#### 第二步：调用 `POST /bot-api/notes` 真正保存笔记
```
POST /bot-api/notes
{
    "title": "从 AI 回复中提取的标题",
    "content": "chat 接口返回的 response 内容（AI 整理后的摘要）",
    "category": "根据内容判断：work/study/life/general",
    "tags": "根据内容提取的标签，逗号分隔",
    "source_url": "原始链接 URL"
}
```

#### 标题提取规则
从 `response` 内容中提取标题，优先级：
1. Markdown `# ` 开头的标题行
2. 内容第一行的主题概括
3. 原网页标题

#### 判断用户是否有保存意图的关键词
以下关键词表示用户希望保存：`保存`、`存到知识库`、`记笔记`、`记下来`、`整理保存`、`存入笔记`、`收藏`、`存起来`、`帮我记`、`帮我存`、`保存笔记`

#### 完整示例
用户说：`https://example.com/python-tips 帮我整理保存到知识库`

1. 先调用 chat：
```json
POST /bot-api/chat
{"message": "https://example.com/python-tips 帮我整理保存到知识库", "user_id": "张三"}
```
得到 response: `# Python 实用技巧\n\n## 1. 列表推导式\n...`

2. 再调用 notes 保存：
```json
POST /bot-api/notes
{"title": "Python 实用技巧", "content": "# Python 实用技巧\n\n## 1. 列表推导式\n...", "category": "study", "tags": "Python,编程技巧"}
```

3. 最终回复用户：`✅ 已保存笔记：Python 实用技巧`

> ⚠️ **绝对不能省略第二步！** chat 接口的 AI 回复中可能会说"已保存"，但那是 AI 的话术，实际数据库中并没有保存。必须显式调用 `POST /bot-api/notes` 才能真正写入数据库。

---

### 执行流程

1. **对话请求**：使用 `curl` 或 `fetch` 调用 API
2. **上下文维护**：记住最后一次返回的 `conversation_id`，后续请求带上它以维持对话上下文
3. **链接 + 保存意图**：**必须** 先调用 `/bot-api/chat` 获取分析内容，再调用 `POST /bot-api/notes` 保存笔记（参见上方两步流程）
4. **结果展示**：
   - 普通对话：直接展示 `response` 字段
   - 如有搜索操作：在回复前加上 `🔍 已搜索: xxx`
   - 如有记账：在回复后加上 `💰 已记录/已修改/已删除: 类型 ¥金额 (分类)`
   - 如有 URL 分析：在回复前加上 `🔗 已抓取: 网页标题`
   - 如有笔记保存：在回复后加上 `📝 已保存笔记: 标题`
   - 笔记列表：以编号列表展示标题和分类
   - 账单列表：以表格形式展示日期、类型、金额、分类、描述，末尾附上汇总

### 帮助信息

当用户发送 `/帮助` 或 `/help` 时，回复：

```
🤖 学习助手指令：

💬 直接发消息 → 和 AI 对话
/新对话 → 开始新对话（清除上下文）
/搜索 内容 → 强制联网搜索
/记笔记 内容 → 将内容保存为笔记
/笔记列表 → 查看所有笔记
/搜笔记 关键词 → 搜索笔记
/账单 → 查看本月收支
/历史 → 查看最近对话列表
/帮助 → 显示此帮助

💡 也支持自然语言：
• 发送链接 → 自动抓取分析
• 发送链接 + "保存" → 抓取分析 + 自动保存到知识库
• 说"帮我搜xxx" → 自动联网搜索
• 说"花了30块吃饭" → 自动记账
• 说"记一下这个知识点" → 保存笔记
• 说"查一下我的笔记" → 搜索知识库
• 说"这个月花了多少钱" → 查询账单
```

## 注意事项

1. **Token 安全**：不要在聊天中泄露 API Token
2. **数据隔离**：每个 user_id 只能操作自己的数据，服务端会校验
3. **数据归属**：记账和知识库数据归属于 `X-Bot-User` 所指向的用户。OpenClaw 操作时，必须使用 user-check 返回的 `bot_user_id` 值，确保数据写入用户在网站注册的真实账号
4. **账号匹配规则**：系统会按以下顺序匹配用户 — ① 精确匹配用户名 → ② 匹配 `bot_` 前缀用户 → ③ 自动创建 `bot_` 用户。所以传入注册的真实用户名可以直接操作该账号
5. **超时处理**：AI 对话接口超时设为 120 秒（AI 思考需要时间），其他接口 30 秒即可
6. **消息长度**：单条消息建议不超过 4000 字
7. **对话上下文**：系统会自动维护对话上下文，超过 16 条会自动压缩摘要
8. **记账格式**：通过对话接口用自然语言说即可自动记账，或通过 `/bot-api/finance` 接口手动添加
9. **分页**：笔记和记账列表默认每页 20 条，最大 50 条
10. **⚠️ 链接保存**：`/bot-api/chat` 接口 **不会** 自动保存笔记！AI 回复中可能"说"已保存，但那只是话术。必须在 chat 之后显式调用 `POST /bot-api/notes` 才能真正保存到数据库。这是最容易出错的地方，务必记住两步操作流程。
